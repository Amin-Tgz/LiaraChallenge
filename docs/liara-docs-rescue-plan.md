...
- صفحهٔ اول خودِ گفت‌وگو است. ارسال سؤال روت را عوض نمی‌کند: پرسش‌های مرتبط به‌صورت درون‌خطی نشان داده می‌شوند و تا وقتی کاربر نگوید کافی نبودند، هیچ فراخوانی مدل انجام نمی‌شود. اگر نتیجه‌ای بالای آستانه نبود، مستقیم وارد گفت‌وگو می‌شود.
- ورودی‌های Skill و MCP در سایدبار ثابت‌اند، نه یک مرحله در مسیر.
...
- گفت‌وگو سقف نوبت ندارد. نوبت‌های خارج از پنجرهٔ بازپخش خام، در بک‌اند خلاصه می‌شوند تا هزینه با طول گفت‌وگو رشد نکند؛ کاربر چیزی نمی‌بیند و فقط سؤال بعدی‌اش را می‌پرسد. `MAX_CONVERSATION_TURNS` فقط سقف محافظتی ضدسوءاستفاده است.
- در حین پاسخ، مراحل واقعی جست‌وجوی عامل (ابزار، عبارت جست‌وجو، تعداد نتیجه و بالاترین شباهت) نمایش داده شود. مرحله‌ای که اتفاق نیفتاده، نمایش داده نمی‌شود.
- زیر هر پاسخ، رأی مفید/غیرمفید با علت گرفته شود؛ صفحات استنادشدهٔ همان پاسخ در سمت سرور به رأی پیوست می‌شوند تا «کدام بخش مستندات مشکل دارد» قابل کوئری باشد.
...
فایل دانلودشدنی باید مستقل و مفید باشد: لینک مستندات رسمی و مخزن منبع، نقشهٔ
سطح‌بالای سرویس‌ها و صفحات، schema مؤلفه‌های MDX (از جمله `Section`، `Tabs`،
`Step` و هشدارها)، روش استخراج محتوای JSX، قواعد انتخاب route و قرارداد پاسخ
کامل و دقیق را در خود داشته باشد. اتصال MCP مسیر retrieval ترجیحی است؛ اگر MCP
در دسترس نبود، Agent می‌تواند همان منابع رسمی را مستقیم بخواند، اما همچنان حق
حدس‌زدن یا ساختن claim بدون evidence را ندارد.

Skill باید دارای README نصب، نسخه و تست نمونه باشد و فایل دانلودشده با validator
مهارت بررسی شود.
