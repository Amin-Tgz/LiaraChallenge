...
- Whether `pg_trgm` is available on Liara's managed Postgres. Determines whether fuzzy Persian lexical matching is added; `tsvector` alone is sufficient to ship, so this changes neither the specs nor the task breakdown. **Partially answered:** on the local `pgvector/pgvector:0.8.0-pg17` image (server 17.6) `pg_trgm` is *available* at version 1.6 but not installed, and `vector` is installed at 0.8.0. The Liara side is still unverified and remains task 1.4: the CLI exposes no way to read a managed database's connection string (`liara db` has only create/list/remove/resize/start/stop), and `DATABASE_URL` is not set on `liara-rescue-api` or `liara-rescue-worker`, so no client can reach the production instance yet. Verifying it requires the production `DATABASE_URL` to be set in the Liara panel first.
